(function($) {
	...
	$.fn.simple_slider = function(settings) {
		...
		function _goLeft(){
			...
			if (firstPos > 0){
				//remove the last one
				$(selector + " img:eq("+ (firstPos + options.display - 1) + ")").fadeOut("fast", function (){
					firstPos--;
					//add one from the beginning
					$(selector + " img:eq("+ (firstPos) +")").fadeIn("fast",function(){
						isRunning = false;
						_checkNavigation();
					});
				});				
		...
		function _goRight(){
			...
			if (firstPos + options.display < totalCount){
				...
				$(selector + " img:eq("+ firstPos +")").fadeOut("fast", function (){
					firstPos++;
					//add one from the end
					$(selector + " img:eq("+ (firstPos + options.display - 1) +")").fadeIn("fast",function(){
						isRunning = false;
						_checkNavigation();
					});
				});				
